...
$(document).on('click','#pay-method-radio',function (){
   var checkedRadio=$('#pay-method-radio input:radio:checked').val();
   $('#pay_method').val(checkedRadio);
});
    ...
    if(pay_method=='card'){
        ...
        IMP.request_pay({
            ...
            if (rsp.success) {
                ...
                addOrderAjax(pay_method,rsp.imp_uid).then(addOrderFoodListAjax).then(successFunction).catch(errorFunction);
            } else {
                var msg = '결제에 실패하였습니다.';
        ...
        addOrderAjax(pay_method,"null").then(addOrderFoodListAjax).then(successFunction).catch(errorFunction);
    }
...
function addOrderAjax(pay_method,imp_uid){
    return new Promise(function (resolve, reject) {
        var orderJsonData =createOrderJsonData(pay_method,imp_uid);
...
function createOrderJsonData(pay_method,imp_uid){
    if(pay_method=="card"){
        var pg='inicis';
    }else{
        pg="null";
    ...
    var food_name_qty_div=food_list[0].getElementsByClassName('order_food_name_qty')[0];
    var food_name=food_name_qty_div.getElementsByClassName('food_name')[0].innerText;
    var food_qty=food_name_qty_div.getElementsByClassName('food_qty')[0].innerText;
    obj.order_content=food_name+" x "+food_qty;
    if(food_list.length > 1){
        obj.order_content+=" 외 "+String(food_list.length-1)+"개";
    }
